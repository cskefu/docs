...
MySQL 容器启动后，还需要创建春松客服数据库，该过程是在数据库上执行 SQL 文件，执行该过程，是通过数据库客户端连接后执行 SQL 文件。
...
2）在上一步的数据库 `cosinee` 中，创建表及数据：[002.mysql-create-schemas.sql](https://github.com/cskefu/cskefu/blob/develop/contact-center/config/sql/002.mysql-create-schemas.sql)
